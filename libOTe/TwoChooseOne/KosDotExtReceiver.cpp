...
namespace osuCrypto
{
    void KosDotExtReceiver::setUniformBaseOts(gsl::span<std::array<block, 2>> baseOTs)
    {
    ...
    coproto::Proto KosDotExtReceiver::receive(
        ...
        PRNG& prng)
    {
        ...
        {
            KosDotExtReceiver& ot;
            const BitVector& choices;
            gsl::span<block> messages;
            PRNG& prng;
            Proto(KosDotExtReceiver& o, const BitVector& c, gsl::span<block> m, PRNG& p)
                : ot(o)
                , choices(c)
                , messages(m)
                , prng(p)
            ...
            u64 numOtExt, numSuperBlocks, numBlocks, step, superBlkIdx;
            block seed, theirSeed, offset;
            Commit myComm;
            BitVector choices2;
            span<block> choiceBlocks;
            Matrix<u8> t0, messageTemp;
            Matrix<u8>::iterator mIter;
            std::vector<block> uBuff;
            block* uIter, * uEnd, * cIter, * tIter;
            PRNG commonPrng, codePrng;
            LinearCode code;
            std::vector<std::array<block, 4>> correlationData;
            ...
            coproto::error_code resume() override
            {
                ...
                ot.setTimePoint("KosDot.recv.start");
                ...
                t0.resize(ot.mGens.size(), superBlkSize * sizeof(block));
                ...
                uBuff.resize(step * ot.mGens.size() * superBlkSize);
                ...
                for (superBlkIdx = 0; superBlkIdx < numSuperBlocks; ++superBlkIdx)
                {
                    ...
                    if (uIter == uEnd)
                    {
                        ...
                        step = std::min<u64>(numSuperBlocks - superBlkIdx - 1, (u64)commStepSize);
                ...
                ot.setTimePoint("KosDot.recv.transposeDone");
                ...
                ot.setTimePoint("KosDot.recv.cncSeed");

                codePrng.SetSeed(theirSeed);
                code.random(codePrng, ot.mGens.size(), 128);

                // this buffer will be sent to the other party to prove we used the
                // same value of r in all of the column vectors...
                correlationData.resize(2);
                {
                ...
                ot.setTimePoint("KosDot.recv.done");
