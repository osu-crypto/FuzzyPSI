...
namespace osuCrypto
{
    ...
    coproto::Proto IknpOtExtReceiver::receive(const BitVector& choices, span<block> messages, PRNG& prng)
    {
        ...
        {
            IknpOtExtReceiver& ot;
            const BitVector& choices;
            span<block> messages;
            PRNG& prng;
            RProto(IknpOtExtReceiver& recv, const BitVector& c, span<block> m, PRNG& p)
                :ot(recv)
                , choices(c)
                , messages(m)
                , prng(p)
        ...
        return coproto::makeProto<RProto>(*this, choices, messages, prng);
    }
