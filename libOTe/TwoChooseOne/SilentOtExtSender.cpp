...
namespace osuCrypto
{
    ...
    coproto::Proto SilentOtExtSender::send(
        span<std::array<block, 2>> messages,
        PRNG& prng)
    {
        struct Proto : coproto::NativeProto
        {
            SilentOtExtSender& ot;
            span<std::array<block, 2>> messages;
            PRNG& prng;

            Proto(SilentOtExtSender& o,
            ...
            {
                CP_BEGIN();
                CP_AWAIT(ot.silentSend(messages, prng));
                correction.resize(messages.size());
                data = correction.getArrayView<u8>();
                CP_RECV(data);
    ...
    coproto::Proto SilentOtExtSender::silentSend(span<std::array<block, 2>> messages, PRNG& prng)
    {
        struct Proto : coproto::NativeProto
        {
            SilentOtExtSender& ot;
            ...
            PRNG& prng;
            u64 numThreads = 1;
            Proto(SilentOtExtSender& o, span<std::array<block, 2>> m, PRNG& p)
                : ot(o)
                , messages(m)
                , prng(p)
            {}
            ...
            Matrix<block> rT;
            MultType type = MultType::QuasiCyclic;
            block delta;
            coproto::error_code resume() override
            {
                CP_BEGIN();

                if (ot.isConfigured() == false)
                {
                    // first generate 128 normal base OTs
                    ot.configure(messages.size(), 2, 128, numThreads);
                }

                if (static_cast<u64>(messages.size()) > ot.mN)
                    throw std::invalid_argument("messages.size() > n");

                if (ot.mGen.hasBaseOts() == false)
                {
                    // make sure we have IKNP base OTs.
                    if (ot.mIknpSender.hasBaseOts() == false)
                        CP_AWAIT(ot.genBaseOts(prng));

                    CP_AWAIT(ot.genSilentBaseOts(prng));
                }

                ot.setTimePoint("sender.expand.start");

                rT.resize(128, ot.mN2 / 128, AllocType::Uninitialized);

                delta = prng.get();
                CP_AWAIT(ot.mGen.expand(numThreads, delta, prng, rT, true, false));
                ot.setTimePoint("sender.expand.pprf_transpose");


                if (ot.mDebug)
                {
                    CP_AWAIT(ot.checkRT(rT));
                }
                ...
                {
                case MultType::Naive:
                    ot.randMulNaive(rT, messages);
                    break;
                case MultType::QuasiCyclic:
                    ot.randMulQuasiCyclic(rT, messages, numThreads);
                    break;
                default:
                    break;
                }
